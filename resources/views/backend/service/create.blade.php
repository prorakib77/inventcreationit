            ...
            <form action="{{ route('service.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex flex-col space-y-2">
                    <input type="text" placeholder="Plan Name" name="service_name"
                        class="w-full input input-bordered input-accent" value="{{ old('service_name') }}" />
                    @error('service_name')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror

                    <textarea class="w-full textarea  textarea-accent" placeholder="Plan details" name="service_description" rows="3">{{ old('service_description') }}</textarea>
                    ...
                    <div x-data="{ packages: [''] }" class="space-y-2">
